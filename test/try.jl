...
s="""
function _fielddescr1(::Type{BStruct{T}},::Val{s}) where {T<:NamedTuple, s}
    shift = 0
...
s="@inline Base.getproperty(x::BStruct{T},Val{s}) where {T<:NamedTuple, s} = _convert($type,_get(reinterpret(UInt64,x),$shift,$bits))"

julia> Meta.parse(s)

:(#= none:1 =# @inline (Base.getproperty(x::BStruct{T}, Val{s}) where {T <: NamedTuple, s}) = begin
              #= none:1 =#
